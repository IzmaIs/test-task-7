...
import {StatusButtons} from "../types/StatusButtons";

const AdCard: FC<AdsBody> = (props) => {

    const [statusButtons, setStatusButtons] = useState<StatusButtons>({
        comparison: false,
        wishlist: false
    })
    ...
    return (
        <div className='card'>
            <div className='card-image'>
                {
                    props.seen &&
                    <div className='card-image-alert__viewed'>
                ...
                <div className='svg-items'>
                    <svg
                        ...
                        className={`svg-actions ${statusButtons.comparison ? 'svg-viewed' : ''}`}
                        onClick={() => setStatusButtons(prevState => ({
                            ...prevState,
                            comparison: !statusButtons.comparison
                        }))}
                    >
                        <path
                    ...
                    <svg
                        ...
                        viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg"
                        className={`svg-actions ${statusButtons.wishlist ? 'svg-viewed' : ''}`}
                        onClick={() => setStatusButtons(prevState => ({
                            ...prevState,
                            wishlist: !statusButtons.wishlist
                        }))}
                    >
                        <path
            ...
            <div className={`card-footer${props.seen ? '__viewed' : ''}`}>
                <div className='card-footer-first-item'>
                    <s className='card-footer-first-item__old-price'>{props.oldPrice} ₽</s>
                    <div className='card-footer-first-item-icons'>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                             className={props.seen ? 'svg-viewed' : ''}>
                            <path
                ...
                <div className='card-footer-second-item'>
                    <span className='card-footer-second-item__new-price'>{props.price} ₽</span>
                    <span className='card-footer-second-item__product-name'>{props.title}</span>
                </div>
                <div className='card-footer-third-item'>
                    <span className='card-footer-third-item__city'>{props.locality}</span>
                    <span
                        className='card-footer-third-item__time'>{dateConvert(props.date).date}, {dateConvert(props.date).hours}:{dateConvert(props.date).minutes}</span>
                </div>
            </div>
        </div>
    );
